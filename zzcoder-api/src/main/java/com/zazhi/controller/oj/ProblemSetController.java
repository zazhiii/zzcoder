...
public class ProblemSetController {
    ...
    @GetMapping("/public")
    public Result<PageResult<ProblemSetPageVO>> pagePublicProblemSet(@RequestParam(value = "page", defaultValue = "1") Integer page,
                                                                     @RequestParam(value = "size", defaultValue = "10") Integer size,
                                                                     @RequestParam(value = "title", required = false) String title) {
        log.info("分页查询公开题单, page: {}, size: {}, title: {}", page, size, title);
        return Result.success(problemSetService.pagePublicProblemSet(page, size, title));
    }

    @Operation(summary = "查询我的所有题单")
    @GetMapping("/my")
    @RequiresAuthentication
    public Result<List<ProblemSetPageVO>> listMyProblemSet() {
        return Result.success(problemSetService.listMyProblemSet());
    }
